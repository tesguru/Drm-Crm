...
protected function buildRawMessage(
    ...
    // 1. Create a "Client-Side" Message-ID
    // Using 16-byte hex makes it look like it was generated by a real mail app
    $domain = substr(strrchr($from, "@"), 1) ?: 'gmail.com';
    $messageId = sprintf('<%s@%s>', bin2hex(random_bytes(16)), $domain);

    // 2. Encode Subject (Standard RFC 2047)
    $encodedSubject = "=?UTF-8?B?" . base64_encode($subject) . "?=";

    // 3. Build Header Array
    // Note: The order matters for some older filters
    $headers = [
        "MIME-Version: 1.0",
        "Date: " . date('r'), // RFC 2822 date is MANDATORY
        "Message-ID: {$messageId}",
        "Subject: {$encodedSubject}",
        "From: \"{$senderName}\" <{$from}>",
        "To: <{$to}>",
        "X-Mailer: iPhone Mail (21G72)", // Mimics a legitimate mobile device
        "Content-Type: " . ($html ? "text/html" : "text/plain") . "; charset=UTF-8",
        "Content-Transfer-Encoding: quoted-printable", // Best for deliverability
    ];
    ...
    // 4. Encode the body
    // quoted-printable prevents long lines from breaking the MIME structure
    $encodedBody = quoted_printable_encode($body);

    // 5. Create the full RFC 2822 string
    $mime = implode("\r\n", $headers) . "\r\n\r\n" . $encodedBody;

    // 6. Base64URL Encode (Gmail API specific: URL-safe, no padding)
